...
Data retrieval and modifications are done with sqlite3
module by issuing SQL statements.
...
To get a row with specific name equal to some value::
    ...
    cursor = conn.cursor()
    cursor.execute(
        'SELECT * from presidents where name=:name',
        {name:'Yeltsin'}
    )
...
To get amount of records in table TABLE1,
use `select count(*) from TABLE1` query.


Please refer to this documents for more information about
...
then
 ...
 - all above mentioned calls should reflect most recent data. If data
 in table changed after you created collection instance,
...
When writing tests, it's not always neccessary to mock
database calls completely. Use supplied example.sqlite file
