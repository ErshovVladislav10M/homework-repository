...
def read_magic_number(path: str, encoding="utf-8", errors="ignore") -> bool:
    with open(path, encoding=encoding, errors=errors) as fi:
        try:
            return 0 <= float(fi.readline()) < 3
        except ValueError:
            print("В первой строке не число")
